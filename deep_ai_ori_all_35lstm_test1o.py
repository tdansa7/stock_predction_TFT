...
# モデルの学習
nf.fit(df=train_df)
nf.save(path='./checkpoints/test_runTFT_day/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for j in range(1, days_pred):
    ...
    last_price = train_df.loc[len(train_df)-1,"Open"]
    close_price = df.loc[len(df)-j,"y"]

    #price_data = last_price["y"].values
    close_pricepre =  Y_hat_df.head(1)["TFT"].values
