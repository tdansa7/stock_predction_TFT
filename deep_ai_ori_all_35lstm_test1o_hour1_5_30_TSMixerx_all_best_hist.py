...
for codem in listcode:
    ...
    df['OpenB'] = (df['Open']-df['Close'].shift(1))/df['Open']
    for i in range(1, 6):
        #df[f'hour{i}'] = (df[f'hour{i}']-df['Open'])/df['Open']
        df[f'hourB{i}'] = (df[f'hour{i}']-df['Close'].shift(1))/df['Open']
    
    df['CloseB'] = df['Close'] /  df['Open']
    ...
    for row in df.itertuples():
        data.append({'ds': df.at[0, 'datetime']+pd.Timedelta(days=j), 
            'y': df.at[row.Index, 'CloseB'], 
            'unique_id': codem,
            'Open':df.at[row.Index, 'OpenB'] ,
            "RSI": df.at[row.Index, 'RSI'],
            'MACD': df.at[row.Index, "MACD5"],
            'MA': df.at[row.Index, "MACD2"],
            'hour1': df.at[row.Index, "hourB1"],
            'hour2': df.at[row.Index, "hourB2"],
            'hour3': df.at[row.Index, "hourB3"],
            'hour4': df.at[row.Index, "hourB4"],
            'hour5': df.at[row.Index, "hourB5"],
            'SAR': df.at[row.Index, 'SAR'],
            'ave': df.at[row.Index, 'ave'],
        })
        j = j+1
    ...
    # モデルの学習
    nf5.fit(df=train_df)
    nf5.save(path='./checkpoints/test_TSMixerx_day_hour5_on'+codem+'/',
            model_index=None, 
            overwrite=True,
            save_dataset=True)
    #nf5 = NeuralForecast.load(path='./checkpoints/test_TSMixerx_day_hour5_on'+codem+'/')
    ...
    for unique_id in test_df['unique_id'].unique():
        ...
        y_pred = Y_hat_df[Y_hat_df.index == unique_id]["TSMixerx"]
    ...
    ax.plot(predicted["ds"], predicted["TSMixerx"], label='Predicted')
    # タイトルとラベルの設定
    ...
    rnn_params = {
        ...
        'hist_exog_list':["RSI",'MACD',"MA",'Open','hour1','hour2','hour3','hour4'],
    ...
    # モデルの学習
    nf4.fit(df=train_df)
    nf4.save(path='./checkpoints/test_TSMixerx_day_hour4_on'+codem+'/',
            model_index=None, 
            overwrite=True,
            save_dataset=True)
    ...
    rnn_params = {
        ...
        'hist_exog_list':["RSI",'MACD',"MA",'Open','hour1','hour2','hour3'],
    ...
    # モデルの学習
    nf3.fit(df=train_df)
    nf3.save(path='./checkpoints/test_TSMixerx_day_hour3_on'+codem+'/',
            model_index=None, 
            overwrite=True,
            save_dataset=True)
    ...
    rnn_params = {
        ...
        'hist_exog_list':["RSI",'MACD',"MA",'Open','hour1','hour2'],
    ...
    # モデルの学習
    nf2.fit(df=train_df)
    nf2.save(path='./checkpoints/test_TSMixerx_day_hour2_on'+codem+'/',
            model_index=None, 
            overwrite=True,
            save_dataset=True)
    ...
    rnn_params = {
        ...
        'hist_exog_list':["RSI",'MACD',"MA",'Open','hour1'],
    ...
    # モデルの学習
    nf1.fit(df=train_df)
    nf1.save(path='./checkpoints/test_TSMixerx_day_hour1_on'+codem+'/',
            model_index=None, 
            overwrite=True,
            save_dataset=True)
    ...
    nf_open.save(path='./checkpoints/test_TSMixerx_day_open_1_on'+codem+'/',
            model_index=None, 
            overwrite=True,
            save_dataset=True)
    ...
    for j in range(1, days_pred):
        ...
        Y_hat_df_sub = nf_open.predict(df=train_df1)
        Y_hat_df_open = pd.DataFrame(Y_hat_df_sub)
        Y_hat_df_sub = nf1.predict(df=train_df1)
        Y_hat_df_1 = pd.DataFrame(Y_hat_df_sub)
        Y_hat_df_sub = nf2.predict(df=train_df1)
        Y_hat_df_2 = pd.DataFrame(Y_hat_df_sub)
        Y_hat_df_sub = nf3.predict(df=train_df1)
        Y_hat_df_3 = pd.DataFrame(Y_hat_df_sub)
        Y_hat_df_sub = nf4.predict(df=train_df1)
        Y_hat_df_4 = pd.DataFrame(Y_hat_df_sub)
        Y_hat_df_sub = nf5.predict(df=train_df1)
        ...
        ave_price = close_price.iloc[0]['ave']
        close_price = close_price.iloc[0]['y']*ave_price


        last_price_ave1 = df.tail(j+1)
        last_price_ave1 = last_price_ave1.head(1)
        last_price_ave1 = last_price_ave1.iloc[0]['ave']
        last_price_close = df.tail(j+1)
        last_price_close = last_price_close.head(1)
        last_price_close = last_price_close.iloc[0]['y'] * last_price_ave1

        last_price_open = train_df1.tail(1)
        last_price_open = ave_price *last_price_open.iloc[0]['Open'] + last_price_close
        last_price_1 = train_df1.tail(1)
        last_price_1 = ave_price *last_price_1.iloc[0]['hour1'] + last_price_close
        last_price_2 = train_df1.tail(1)
        last_price_2 = ave_price *last_price_2.iloc[0]['hour2'] + last_price_close
        last_price_3 = train_df1.tail(1)
        last_price_3 = ave_price *last_price_3.iloc[0]['hour3'] + last_price_close
        last_price_4 = train_df1.tail(1)
        last_price_4 =ave_price *last_price_4.iloc[0]['hour4'] + last_price_close
        last_price_5 = train_df1.tail(1)
        last_price_5 = ave_price *last_price_5.iloc[0]['hour5'] + last_price_close
        

        #price_data = last_price["y"].values
        close_pricepre =  Y_hat_df_open.head(1)["TSMixerx"].values
        close_pricepre_open = close_pricepre[0] * ave_price
        close_pricepre =  Y_hat_df_1.head(1)["TSMixerx"].values
        close_pricepre_1 = close_pricepre[0]* ave_price
        close_pricepre =  Y_hat_df_2.head(1)["TSMixerx"].values
        close_pricepre_2 = close_pricepre[0]* ave_price
        close_pricepre =  Y_hat_df_3.head(1)["TSMixerx"].values
        close_pricepre_3 = close_pricepre[0]* ave_price
        close_pricepre =  Y_hat_df_4.head(1)["TSMixerx"].values
        close_pricepre_4 = close_pricepre[0]* ave_price
        close_pricepre =  Y_hat_df_5.head(1)["TSMixerx"].values
        close_pricepre_5 = close_pricepre[0]* ave_price
        ...
        if(last_price_1 < close_pricepre_1):
            if(last_price_2 < close_pricepre_2):
                ...
                listdata.append({'No':j,
                    ...
                    'preClose4': close_pricepre_4,
                    'preClose5': close_pricepre_5,
                    'preprice': last_price_2-last_price_1
                })
        else:
            if(last_price_2 > close_pricepre_2):
                if(last_price_3 > close_pricepre_3):
