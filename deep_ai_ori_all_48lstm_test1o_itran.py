...
listcode = [7201,7202,7203,7205,7211,7261,7267]
codem="2809"
path="output_m\\list_"
path2="output\\list_"
days_pred =100
...
rnn_params = {
    ...
    'd_ff':  256,#1024
    'batch_size' : 128,
    #'horizon_weight': horizon_weight,  # ここに horizon_weight を追加
}
# モデルのインスタンス
...
nf.save(path='./checkpoints/test_iTransformer_60'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for unique_id in test_df_a['unique_id'].unique():
    ...
    # 予測値を取得
    y_pred = test_F[test_F.index == unique_id]["iTransformer"].values
