...
rnn_params = {
    ...
    'hist_exog_list':['Open',"RSI",'MACD','hour1'],
...
# モデルの学習
#nf.fit(df=train_df)
#nf.save(path='./checkpoints/test_runTFT_day_hour1/',
#        model_index=None, 
#        overwrite=True,
#        save_dataset=True)
nf_1 = NeuralForecast.load(path='./checkpoints/test_runTFT_day/')
nf = NeuralForecast.load(path='./checkpoints/test_runTFT_day_hour1/')
# 予測の実施
...
for j in range(1, days_pred):
    ...
    Y_hat_df_sub = nf_1.predict(df=train_df1)
    Y_hat_df_1 = pd.DataFrame(Y_hat_df_sub)
    Y_hat_df_sub = nf.predict(df=train_df1)
    ...
    last_price_1 = last_price_1.iloc[0]['Open']
    last_price_2 = train_df1.tail(1)
    last_price_2 = last_price_2.iloc[0]['hour1']
    #print(last_price)
    close_price = df.tail(j)
    close_price = close_price.head(1)
    close_price = close_price.iloc[0]['hour1']
    #print(close_price)
    ...
    if(last_price_1 < close_pricepre_1):
        ...
        if(last_price_2 > close_pricepre_2):
            ...
            listdata.append({'No':j,
                ...
                'preClose1': close_pricepre_1,
                'preClose2': close_pricepre_2,
                'preprice': last_price_1-close_price
            })
        else:
            sumdata = sumdata - last_price_2+last_price_1
            listdata.append({'No':j,
                ...
                'preClose1': close_pricepre_1,
                'preClose2': close_pricepre_2,
                'preprice': last_price_1-last_price_2
            })
