...
class CustomFEDformer(TSMixerx):
...
for codem in listcode:
    ...
    df['OpenB'] = df['Open']-df['Close'].shift(1)
    for i in range(1, 6):
        #df[f'hour{i}'] = (df[f'hour{i}']-df['Open'])/df['Open']
        df[f'hourB{i}'] = df[f'hour{i}']-df['Close'].shift(1)
    ...
    for j in range(1, days_pred):
        ...
        last_price_close = last_price_close.iloc[0]['y']
        last_price_open = test_df.head(1)
        last_price_open = last_price_open.iloc[0]['Open'] + last_price_close
        last_price_1 = test_df.head(1)
        last_price_1 = last_price_1.iloc[0]['hour1'] + last_price_close
        last_price_2 = test_df.head(1)
        last_price_2 = last_price_2.iloc[0]['hour2'] + last_price_close
        last_price_3 = test_df.head(1)
        last_price_3 = last_price_3.iloc[0]['hour3'] + last_price_close
        last_price_4 = test_df.head(1)
        last_price_4 = last_price_4.iloc[0]['hour4'] + last_price_close
        last_price_5 = test_df.head(1)
        last_price_5 = last_price_5.iloc[0]['hour5'] + last_price_close
