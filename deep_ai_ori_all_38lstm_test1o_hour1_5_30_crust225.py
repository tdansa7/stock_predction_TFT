...
for codem in listcode_stock:
    ...
    df['open_by_close_up'] = df['open_by_close'].apply(lambda x: 1 if x > 1.01 else 0)
    df['open_by_close_down'] = df['open_by_close'].apply(lambda x: 1 if x < 0.99 else 0)
    #for i in range(1, 11):
...
print(input_size)
print(len(list_out))
hidden_layers = [10*input_size, 5*output_size, output_size]
