...
def apply_operation_up(row):
    col_prefix = row['max_column_up'][:4]  # 最初の1文字を抜き出す
    if col_prefix in operations:
        return operations[col_prefix](row['up_buy'])
    else:
        return row['up_buy']  # デフォルトではそのまま返す

def apply_operation_down(row):
    col_prefix = row['max_column_down'][:4]  # 最初の1文字を抜き出す
    if col_prefix in operations:
        return operations[col_prefix](row['down_sell'])
    else:
        return row['down_sell']  # デフォルトではそのまま返す
path="stock_list\\"
path2="output\\list_"
...
merged_df = merged_df.drop(columns=['data','ds'])
