...
df['Open'] = (df['Open']-df['Openave'])/df['Openave']
df['Close'] = (df['Close']-df['Openave'])/df['Openave']
df["hour1"] = (df["hour1"]-df['Openave'])/df['Openave']
df["hour2"] = (df["hour2"]-df['Openave'])/df['Openave']
df["hour3"] = (df["hour3"]-df['Openave'])/df['Openave']
df["hour4"] = (df["hour4"]-df['Openave'])/df['Openave']
df["hour5"] = (df["hour5"]-df['Openave'])/df['Openave']
...
for row in df.itertuples():
    data.append({'ds': df.at[0, 'datetime']+pd.Timedelta(days=j), 
        ...
        'Openave':df.at[row.Index, 'Openave']
    })
    j = j+1
...
# モデルの学習
nf5.fit(df=train_df)
nf5.save(path='./checkpoints/test_runTFT_day_hour5_mean'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for unique_id in test_df['unique_id'].unique():
    ...
    y_pred = Y_hat_df[Y_hat_df.index == unique_id]["TFT"]
...
ax.plot(predicted["ds"], predicted["TFT"], label='Predicted')
# タイトルとラベルの設定
...
# モデルの学習
nf4.fit(df=train_df)
nf4.save(path='./checkpoints/test_runTFT_day_hour4_mean'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
# モデルの学習
nf3.fit(df=train_df)
nf3.save(path='./checkpoints/test_runTFT_day_hour3_mean'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
# モデルの学習
nf2.fit(df=train_df)
nf2.save(path='./checkpoints/test_runTFT_day_hour2_mean'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
# モデルの学習
nf1.fit(df=train_df)
nf1.save(path='./checkpoints/test_runTFT_day_hour1_mean'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
nf_open.save(path='./checkpoints/test_runTFT_day_open_1_mean'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for j in range(1, days_pred):
    ...
    #print(last_price)
    close_price1 = df.tail(j)
    close_price1 = close_price1.head(1)
    close_price = close_price1.iloc[0]['y']
    Openave = close_price1.iloc[0]['Openave']
    ...
    close_pricepre =  Y_hat_df_open.head(1)["TFT"].values
    close_pricepre_open = close_pricepre[0]
    close_pricepre =  Y_hat_df_1.head(1)["TFT"].values
    close_pricepre_1 = close_pricepre[0]
    close_pricepre =  Y_hat_df_2.head(1)["TFT"].values
    close_pricepre_2 = close_pricepre[0]
    close_pricepre =  Y_hat_df_3.head(1)["TFT"].values
    close_pricepre_3 = close_pricepre[0]
    close_pricepre =  Y_hat_df_4.head(1)["TFT"].values
    close_pricepre_4 = close_pricepre[0]
    close_pricepre =  Y_hat_df_5.head(1)["TFT"].values
