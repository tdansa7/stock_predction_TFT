...
def calculate_difference_up(row):
    ...
    example = row[f'{col_prefix}high']/row[f'{col_prefix}Open']
    if example >1.01:
        anser =  row[f'{col_prefix}Open']*0.01
    else:
        anser = row[f'{col_prefix}Close'] - row[f'{col_prefix}Open']
    return anser
def calculate_difference_down(row):
    col_prefix = row['max_column_down'][:4]
    if row[f'{col_prefix}low']/row[f'{col_prefix}Open'] <0.99:
        anser =  row[f'{col_prefix}Open']*0.01
    else:
        anser = -row[f'{col_prefix}Close'] + row[f'{col_prefix}Open']
    return anser
...
for codem in listcode_stock:
    ...
    df['open_by_high'] = df['high']/df['Open']
    df['open_by_low'] = df['low']/df['Open']
    df['open_by_close_up'] = df['open_by_high'].apply(lambda x: 1 if x > 1.01 else 0)
    df['open_by_close_down'] = df['open_by_low'].apply(lambda x: 1 if x < 0.99 else 0)
    #for i in range(1, 11):
...
for name in listcode_stock:
    list_out.append(name+'open_by_close_up')
    list_clas.append(name+'open_by_close_up')
    list_out.append(name+'open_by_close_down')
    #list_out.append(name+'open_by_close_down')
...
# 'signal' 列に対してフーリエ変換を実施
fourier_transform = np.fft.fft(merged_df[list_sel[0]])

# 周波数の計算
frequencies = np.fft.fftfreq(len(merged_df[list_sel[0]]), 1)
...
result_df['max_column_up'] = result_df[target_columns_up].idxmax(axis=1)
result_df['max_column_down'] = result_df[target_columns_down].idxmax(axis=1)
