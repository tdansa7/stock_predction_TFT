...
rnn_params = {
    ...
    'scaler_type':'robust',
    'max_steps': 1000, 
    'learning_rate':0.0000000000000000000000000000000000001,
    'dropout' : 0,
...
# モデルの学習
nf5.fit(df=train_df)
nf5.save(path='./checkpoints/test_TimesNet_day_hour5_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for unique_id in test_df['unique_id'].unique():
    ...
    y_pred = Y_hat_df[Y_hat_df.index == unique_id]["TimesNet"]
...
ax.plot(predicted["ds"], predicted["TimesNet"], label='Predicted')
# タイトルとラベルの設定
...
# モデルの学習
nf4.fit(df=train_df)
nf4.save(path='./checkpoints/test_TimesNet_day_hour4_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
# モデルの学習
nf3.fit(df=train_df)
nf3.save(path='./checkpoints/test_TimesNet_day_hour3_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
# モデルの学習
nf2.fit(df=train_df)
nf2.save(path='./checkpoints/test_TimesNet_day_hour2_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
# モデルの学習
nf1.fit(df=train_df)
nf1.save(path='./checkpoints/test_TimesNet_day_hour1_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
nf_open.save(path='./checkpoints/test_TimesNet_day_open_1_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for j in range(1, days_pred):
    ...
    close_pricepre =  Y_hat_df_open.head(1)["TimesNet"].values
    close_pricepre_open = close_pricepre[0]
    close_pricepre =  Y_hat_df_1.head(1)["TimesNet"].values
    close_pricepre_1 = close_pricepre[0]
    close_pricepre =  Y_hat_df_2.head(1)["TimesNet"].values
    close_pricepre_2 = close_pricepre[0]
    close_pricepre =  Y_hat_df_3.head(1)["TimesNet"].values
    close_pricepre_3 = close_pricepre[0]
    close_pricepre =  Y_hat_df_4.head(1)["TimesNet"].values
    close_pricepre_4 = close_pricepre[0]
    close_pricepre =  Y_hat_df_5.head(1)["TimesNet"].values
