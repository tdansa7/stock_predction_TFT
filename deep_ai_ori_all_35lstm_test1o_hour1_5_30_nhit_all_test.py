...
for codem in listcode:
    ...
    rnn_params = {
        ...
        'mlp_units': 3 * [[1024, 1024]],#3 * [[512, 512]]
    ...
    # モデルのインスタンス
    rnn_model = CustomNHITS1(**rnn_params)
    #rnn_model.optimizer = torch.optim.Adam(rnn_model.parameters(), lr=0.0001)
    #optimizer = torch.optim.Adam(self.parameters(), lr=self.learning_rate)
    #rnn_model.lr_schedulers = torch.optim.lr_scheduler.ExponentialLR(rnn_model.optimizer, gamma=0.9),
    ...
    for unique_id in test_df['unique_id'].unique():
        ...
        y_pred = Y_hat_df[Y_hat_df.index == unique_id]["CustomNHITS1"]
    ...
    ax.plot(predicted["ds"], predicted["CustomNHITS1"], label='Predicted')
    # タイトルとラベルの設定
    ...
    # モデルのインスタンス
    rnn_model = CustomNHITS2(**rnn_params)
    ...
    rnn_params = {
        ...
        'batch_size' : 32,
        'stack_types':  ["identity", "identity"],#["identity", "identity", "identity"]
        'n_blocks':  [1, 1],#[1, 1, 1]
        'mlp_units': 2 * [[1024, 1024]],#3 * [[512, 512]]
        'n_pool_kernel_size': [ 2, 1],#[2, 2, 1]
        'n_freq_downsample': [ 2, 1], #[4, 2, 1]
        'pooling_mode': 'AvgPool1d', #['MaxPool1d', 'AvgPool1d']
        'interpolation_mode':  'cubic',#['linear', 'nearest', 'cubic'].
    }
    # モデルのインスタンス
