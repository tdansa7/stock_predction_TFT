...
checkpoint_callback = ModelCheckpoint(
    ...
    dirpath='checkpoints/',  # チェックポイントを保存するディレクトリ
    filename='best-checkpoint',  # チェックポイントのファイル名
    save_top_k=1,  # 最も良いモデルを1つ保存
    mode='min'  # 最小の値が良い
)
# Trainerの設定
trainer = Trainer(
    max_steps=2000,
    callbacks=[checkpoint_callback],  # ここでコールバックを追加
)
...
#    data=train_df,  # 入力データフレーム
#    freq='D',  # データの頻度（日次）
#    max_horizon=horizon,  # 予測ホライゾン（例：24時間）
#    input_size=40 * horizon,  # 入力サイズ（例：過去48時間のデータを使用）
    #lags=[1, 2, 3, 7, 14],  # 使用するラグ（例：過去1日、2日、3日、7日、14日のデータ）
#    target_col='y',  # 予測対象の列名
#    time_col='ds',  # 時間列の列名
#    id_col='unique_id',  # ID列の列名（複数系列の場合）
    #static_cols=['static_feature1', 'static_feature2'],  # 静的特徴量の列名
#    futr_exog_cols=["RSI",'MACD',"MA",'Open','hour1','hour2','hour3','hour4','hour5'],  # 未来の外生変数の列名
    #hist_exog_cols=['hist_exog1', 'hist_exog2'],  # 過去の外生変数の列名
#)
...
rnn_params = {
    ...
    'batch_size' : 8,
    'stack_types':  ["identity", "identity", "identity","identity"],#["identity", "identity", "identity"]
    'n_blocks':  [1, 1, 1,1],#[1, 1, 1]
    'mlp_units': 4 * [[1024, 1024]],#3 * [[512, 512]]
    'n_pool_kernel_size': [4,2, 2, 1],#[2, 2, 1]
    'n_freq_downsample': [4,4, 2, 1], #[4, 2, 1]
    'pooling_mode': 'AvgPool1d', #['MaxPool1d', 'AvgPool1d']
    'interpolation_mode':  'cubic',#['linear', 'nearest', 'cubic'].
    #'lr_scheduler':torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.9),
}
# モデルのインスタンス
rnn_model = CustomNHITS(**rnn_params)
#trainer.fit(rnn_model, datamodule=datamodule)
trainer.fit(rnn_model, datamodule=train_df)

nf5 = NeuralForecast(models=[rnn_model], freq='D')
#print(nf5.lerning_rate)
# モデルの学習
nf5.fit(df=train_df)
#best_model = CustomNHITS.load_from_checkpoint(pathaaa)

# 最小損失のモデルをロード



nf5.save(path='./checkpoints/test_NHITS_day_hour5_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for unique_id in test_df['unique_id'].unique():
    ...
    y_pred = Y_hat_df[Y_hat_df.index == unique_id]["CustomNHITS"]
...
ax.plot(predicted["ds"], predicted["CustomNHITS"], label='Predicted')
# タイトルとラベルの設定
...
#best_model = CustomNHITS.load_from_checkpoint(best_model_path)
#nf4 = NeuralForecast(models=[best_model], freq='D')

nf4.save(path='./checkpoints/test_NHITS_day_hour4_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
#best_model_path = checkpoint_callback.best_model_path
#best_model = CustomNHITS.load_from_checkpoint(best_model_path)
#nf3 = NeuralForecast(models=[best_model], freq='D')

nf3.save(path='./checkpoints/test_NHITS_day_hour3_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
#best_model_path = checkpoint_callback.best_model_path
#best_model = CustomNHITS.load_from_checkpoint(best_model_path)
#nf2 = NeuralForecast(models=[best_model], freq='D')

nf2.save(path='./checkpoints/test_NHITS_day_hour2_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
nf1.save(path='./checkpoints/test_NHITS_day_hour1_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
nf_open.save(path='./checkpoints/test_NHITS_day_open_1_on'+codem+'/',
        model_index=None, 
        overwrite=True,
        save_dataset=True)
...
for j in range(1, days_pred):
    ...
    close_pricepre =  Y_hat_df_open.head(1)["CustomNHITS"].values
    close_pricepre_open = close_pricepre[0]
    close_pricepre =  Y_hat_df_1.head(1)["CustomNHITS"].values
    close_pricepre_1 = close_pricepre[0]
    close_pricepre =  Y_hat_df_2.head(1)["CustomNHITS"].values
    close_pricepre_2 = close_pricepre[0]
    close_pricepre =  Y_hat_df_3.head(1)["CustomNHITS"].values
    close_pricepre_3 = close_pricepre[0]
    close_pricepre =  Y_hat_df_4.head(1)["CustomNHITS"].values
    close_pricepre_4 = close_pricepre[0]
    close_pricepre =  Y_hat_df_5.head(1)["CustomNHITS"].values
    close_pricepre_5 = close_pricepre[0]
    #print(close_pricepre)

    if(last_price_open < close_pricepre_open):
        if(last_price_1 < close_pricepre_1):
                ...
                if(last_price_3 < close_pricepre_3):
                    if(last_price_4 < close_pricepre_4):
                        if(last_price_5 < close_pricepre_5):
                            sumdata = sumdata + close_price-last_price_open
                            listdata.append({'No':j,
                                ...
                                'preprice': close_price-last_price_open
                            })
                        else:
                            sumdata = sumdata + last_price_5-last_price_open
                            listdata.append({'No':j,
                                'Open': last_price_open ,
                                'hour1': last_price_1 ,
                                'hour2': last_price_2 ,
                                ...
                                'preClose4': close_pricepre_4,
                                'preClose5': close_pricepre_5,
                                'preprice': last_price_5-last_price_open
                            })
                    else:
                        sumdata = sumdata + last_price_4-last_price_open
                        listdata.append({'No':j,
                            'Open': last_price_open ,
                            'hour1': last_price_1 ,
                            'hour2': last_price_2 ,
                            ...
                            'preClose4': close_pricepre_4,
                            'preClose5': close_pricepre_5,
                            'preprice': last_price_4-last_price_open
                        })
                else:
                    sumdata = sumdata + last_price_3-last_price_open
                    listdata.append({'No':j,
                        'Open': last_price_open ,
                        'hour1': last_price_1 ,
                        'hour2': last_price_2 ,
                        ...
                        'preClose4': close_pricepre_4,
                        'preClose5': close_pricepre_5,
                        'preprice': last_price_3-last_price_open
                    })
            else:
                sumdata = sumdata + last_price_2-last_price_open
                listdata.append({'No':j,
                    'Open': last_price_open ,
                    'hour1': last_price_1 ,
                    'hour2': last_price_2 ,
            ...
            sumdata = sumdata + last_price_1-last_price_open
            listdata.append({'No':j,
                'Open': last_price_open ,
                'hour1': last_price_1 ,
                'hour2': last_price_2 ,
                ...
                'preClose4': close_pricepre_4,
                'preClose5': close_pricepre_5,
                'preprice': last_price_1-last_price_open
            })
    else:
        if(last_price_1 > close_pricepre_1):
            if(last_price_2 > close_pricepre_2):
